...
with Session(autoflush=False, bind=engine) as db:
    result = (
        db.query(
            Subject.name_subject,
            Question.name_question,
            func.count(Question.question_id).label("Вопрос"),
            round(func.avg(Answer.is_correct) * 100, 2).label("Результат"),
        )
        .join(Question)
        .join(Testing)
        .join(Attempt)
        .join(Answer)
        .group_by(Subject.name_subject, Question.name_question)
        .order_by(
            Subject.name_subject, Answer.результат.desc(), Question.name_question.desc()
        )
        .all()
    )
