...
FROM (SELECT student_name,
             step_id,
             CASE
               WHEN LAG(result) OVER (PARTITION BY student_name, step_id ORDER BY attempt_time) = 'correct' AND
                    result = 'wrong'
                 THEN 'I'
               END AS grp
      FROM student
...
--group 3 not ready
SELECT 
COUNT(step_id) OVER (PARTITION BY student_name, step_id) as aaa, 
COUNT(step_id) OVER (PARTITION BY student_name, step_id, result) as bbb,
 (case 
